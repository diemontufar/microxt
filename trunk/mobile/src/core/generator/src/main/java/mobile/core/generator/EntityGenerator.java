...
public class EntityGenerator {
	...
	public void generateEntities() throws Exception {
		// Query entities
		TypedQuery<EntityTable> queryEntities = 
			JPManager.getEntityManager().createQuery(ENTITY_SQL, EntityTable.class);
	...
	private void generateBuilders(EntityTable entity,
			List<EntityField> lField) throws Exception {
		//Initialize builders
		entityBuilder = null;
	...
	private EntityData generateEntityData(EntityTable entity, List<EntityField> lField, EntityType type) throws Exception{
		//--------------------------------
	...
	private StringBuilder generateEntity(EntityTable entity, List<EntityField> lField, EntityData entityData, EntityType type) {
		...
		for (PropertyData property : entityData.getlProperties()) {
			//Getter
			builder.append("public " + property.getType() 
					+ " get" +  property.getName().substring(0,1).toUpperCase() + property.getName().substring(1) );
			builder.append("() {"+NEW_LINE);
		...
		builder.append("this." + 
				entityData.getlProperties().get(0).name + "=" +
				"(" + entityData.getlProperties().get(0).type + ") pk"+ END_LINE);
		builder.append("}" + NEW_LINE);
	...
	/**
	 * Checks entityField for some necessary import.
	 * If an import is required, it is added to entityData  
	 * 
	 * @param entityData
	 * @param entityField
	 */
	...
	private StringBuilder createPkClassFile(EntityTable fatherEntity, PropertyData pkData, List<EntityField> lPks, EntityType type) {
		//--------------------------------
		...
		// Set entity extend
		// Use to identify the extends combination (MultiCompany MultiLanguage Historical) 
		...
		pkClassFile.append("* The primary key class for the " 
				+ fatherEntity.getPk().getTableId() + " database table." + NEW_LINE);
		pkClassFile.append("*/" + NEW_LINE);
		...
		for (PropertyData property : entityPkData.getlProperties()) {
			// Getter
			pkClassFile.append("public " + property.getType() 
					+ " get" + property.getName().substring(0, 1).toUpperCase() + property.getName().substring(1));
			pkClassFile.append("() {"+NEW_LINE);
	...
	private PropertyData createProperty(EntityField field) {
		...
		annotations.append("@Column(name=\""
				...
				+ ((field.getNullable()) ? ", nullable=true" : ", nullable=false") 
				+ ")" + NEW_LINE);
