...
public class SqlGenerator {
	...
	// Sql for packages
	private String PACKAGES_SQL = "Select distinct e.packageName from EntityTable e where e.pk.companyId in ('ALL',:company)";

	// Sql for entities
	private String ENTITY_SQL = "Select e from EntityTable e where e.pk.companyId in ('ALL',:company) and e.packageName=:packageName ";
	 ...
	 * "r.pk.companyId in ('ALL',:company) " + "and r.tableFrom in ( " +
	...
	public void execute() throws Exception {
		...
		for (Iterator i = lPackages.iterator(); i.hasNext();) {
			...
			System.out
					.println("Writing file " + packageName + "_create.sql...");
			FileUtil.writeFile(
					outputFolder + "/" + packageName + "_create.sql",
					sbSql.toString());
			System.out.println("Writing file " + packageName + "_drop.sql...");
			FileUtil.writeFile(outputFolder + "/" + packageName + "_drop.sql",
					sbDropSql.toString());
		}
	}
	...
	public void dropForeignKeys(String packageName) {
		...
		String finalQuery = RELATIONSHIP_IDS_SQL.replaceAll(":company",
				"'" + COMPANY + "'").replaceAll(":packageName",
				"'" + packageName + "'");
