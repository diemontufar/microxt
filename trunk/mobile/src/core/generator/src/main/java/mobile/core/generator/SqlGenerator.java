...
public class SqlGenerator {
	...
	// Sql for packages
	private String PACKAGES_SQL =
		"Select distinct e.packageName from EntityTable e where e.pk.companyId in ('ALL',:company)";
	
	// Sql for entities
	private String ENTITY_SQL = 
		"Select e from EntityTable e where e.pk.companyId in ('ALL',:company) and e.packageName=:packageName ";
	...
	String RELATIONSHIP_IDS_SQL = 
			"select distinct r.RELATIONSHIP_ID, r.TABLE_FROM from ENTITY_RELATIONSHIP r where "
					...
					+ "and (" 
					+ "r.TABLE_FROM in (Select distinct tt.TABLE_ID from ENTITY_TABLE tt where tt.PACKAGE_NAME = :packageName ) "
					+ "or r.TABLE_TO in (Select distinct CONCAT(tt.TABLE_ID,'_ID') from ENTITY_TABLE tt where tt.PACKAGE_NAME = :packageName and tt.HAS_TABLE_ID='1' ) "
	...
	/*private String RELATIONSHIP_SQL = 
		"Select r from EntityRelationship r where "
			...
			+ "and r.tableFrom in ( "
			+ "Select e.pk.tableId from EntityTable e where e.packageName = :packageName "
			+ ") "
	...
	private String RELATIONSHIP_SQL =
		"Select r.* from ENTITY_RELATIONSHIP r where "
			+ "r.COMPANY_ID in ('ALL',:company) "
			+ "and r.TABLE_FROM in ( "
			+ "Select e.TABLE_ID from ENTITY_TABLE e where e.PACKAGE_NAME = :packageName "
			+ "union "
			+ "Select CONCAT(e.TABLE_ID,'_ID') from ENTITY_TABLE e where e.PACKAGE_NAME = :packageName and HAS_TABLE_ID = '1' "
			+ ") "
	 ...
	 */
	public SqlGenerator(String dbName, String outputFolder){
	...
	public void execute() throws Exception{
		// Query packages
		List lPackages;
		Query query = JPManager.getEntityManager().createQuery(PACKAGES_SQL);
		query.setParameter("company", COMPANY);
		lPackages = query.getResultList();

		for (Iterator i = lPackages.iterator(); i.hasNext();) {
			...
			// Drop script
			dropForeignKeys(packageName);
			dropTables(packageName);
			
			// Create script
			createTables(packageName);
			createRelationships(packageName);
			
			//Write files
			System.out.println("Package: " + packageName);
			System.out.println("Writing file " + packageName +"_create.sql...");
			FileUtil.writeFile(outputFolder + "/" + packageName + "_create.sql", sbSql.toString());
			System.out.println("Writing file " + packageName + "_drop.sql...");
			FileUtil.writeFile(outputFolder + "/" + packageName + "_drop.sql", sbDropSql.toString());			
		}
	}
	
	@SuppressWarnings("rawtypes")
	public void dropForeignKeys(String packageName){
		// Query relationship ids
		List lRelationship;
		String finalQuery = RELATIONSHIP_IDS_SQL.replaceAll(":company", "'"+COMPANY+"'").replaceAll(":packageName", "'"+packageName+"'");
	...
	public void dropTables(String packageName){
		// Query entities
	...
	public void createTables(String packageName) throws Exception{
		// Query entities
		List<EntityTable> lEntity;
		TypedQuery<EntityTable> query = JPManager.getEntityManager().createQuery(ENTITY_SQL, EntityTable.class);
		query.setParameter("company", COMPANY);
	...
	public void createRelationships(String packageName){
		// Query relationships
		List<EntityRelationship> lRelationship;
		String finalQuery = RELATIONSHIP_SQL.replaceAll(":company", "'"+COMPANY+"'").replaceAll(":packageName", "'"+packageName+"'");
