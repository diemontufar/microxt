...
public class ParameterTest implements GeneralProcessor {
	...
	private Message processQuery(Message msg) throws Exception {
		try {
			GeneralProcessor queryProc = (GeneralProcessor) Class.forName("mobile.core.processor.QueryProcessor")
					.newInstance();
	...
	private void processMaintenance(Message msg) {
		try {
			GeneralProcessor maintenanceProc = (GeneralProcessor) Class.forName(
					"mobile.core.processor.MaintenanceProcessor").newInstance();
