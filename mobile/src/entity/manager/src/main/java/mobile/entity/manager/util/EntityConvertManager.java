...
public class EntityConvertManager {
	...
	public static <T> T convertObject(Object pValue, Class<T> pType) {
		try {
			return EntityConvertManager.convertObject(pValue, pType, false);
		} catch (Exception e) {
			throw new Error(e);
		}
	}
	...
	public static <T> T convertObject(Object pValue, Class<T> pType,
		...
		try {
			T oVal = EntityConvertManager.convertValueByType(value, pType);
			if (oVal != null) {
				return oVal;
			}
	...
	private static <T> T convertValueByType(Object value, Class<T> pType)
		...
		if ((pType.getName().compareTo(Blob.class.getName()) == 0)
			...
			return (T) EntityConvertManager.manageBlob(value);
		}
		String sValue = value.toString();
		sValue = EntityConvertManager.correctNumberValue(sValue, pType);
		T oVal = EntityConvertManager.valFromConstructor(pType, sValue);
		if (oVal != null) {
			return oVal;
		}
		if (pType.getName().compareTo(Timestamp.class.getName()) == 0) {
			return (T) EntityConvertManager.convertTimestamp(sValue);
		}
		if (pType.getName().compareTo(Date.class.getName()) == 0) {
			return (T) EntityConvertManager.convertDate(sValue);
		}
